...
import torch
datalist=torch.load('test.pt', weights_only=False)
...
for epoch in range(epochs):
    ...
    for batch in train_dataloader:
        ...
        #print('sigma', sigma)
        #print('batch.stress', batch.stress.view(-1, 3, 3))
        y = batch.y
        loss_e = loss(energies, y)
        loss_f = loss(forces, batch.forces)
        loss_s = loss(sigma, batch.stress.view(-1, 3, 3))
        
        l = loss_e*ene_weight + loss_f*force_weight + loss_s*stress_weight
        l.backward()
        optimizer.step()
        ...
        loss_s_total += loss_s.item()/len(train_dataloader)

    scheduler.step()
    print('epoch: train', epoch, 'loss_total', loss_total, 'loss_e', 
        loss_e_total, 'loss_f', loss_f_total, 'loss_s', loss_s_total)
    ...
    writer.add_scalar('loss_s', loss_s_total, epoch)
    ...
    for batch in test_dataloader:
        ...
        loss_s_total += loss_s.item()/len(test_dataloader)
    print('epoch: test', epoch, 'loss_total', loss_total, 
            'loss_e', loss_e_total, 'loss_f', loss_f_total, 'loss_s', loss_s_total)
    ...
    writer.add_scalar('loss_s_test', loss_s_total, epoch)
...
for batch in test_dataloader:
    ...
    results_stress.extend(sigma.detach().to('cpu').numpy().flatten())
    ref_energy.extend(batch.y.detach().to('cpu').numpy().flatten())
    ref_forces.extend(batch.forces.detach().to('cpu').numpy().flatten())
    ref_stress.extend(batch.stress.detach().to('cpu').numpy().flatten())
...
plt.figure(figsize=(5,5))
plt.gca().set_aspect('equal', adjustable='box')
plt.scatter(ref_stress,results_stress)
plt.xlabel('reference stress')
plt.ylabel('predicted stress')
plt.title('SchNetModel stress')
plt.savefig('stress_schnet_torch_full_stepLR.png')
